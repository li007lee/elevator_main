...
* 将比较器函数设置为列表元素。
*
* 比较器函数用于搜索和排序。如果将NULL作为引用传递给函数，那么comparator将被禁用。
*
* @param l列表操作
* @param comparator_fun指针指向实际比较器函数
* @return 0属性设置成功;-1设置失败
*
...
* 为列表元素设置寻引函数。
*
* 寻引函数是用来寻找元素的。如果将NULL作为对函数的引用传递，则搜索器将被禁用。
*
* @param l列表操作
* @param seeker_fun指针指向实际的探索者函数
* @return 0属性设置成功;-1设置失败
*
* @see element_seeker()
**/
...
/**
*在删除列表条目时需要释放元素数据(默认情况下:不释放)。
*
*[高级属性]
*
* 默认情况下，当元素从列表中删除时，它从列表中消失的实际数据并不是免费的()d。有了这个选项，每个删除都会使元素数据被释放。
*
* 如果将NULL元素插入到列表中，则该函数能够正确处理NULL值。
*
* @param l列表操作
* @param metric_fun指针指向实际的度量函数
* @param copy_data 0:不要释放元素数据(默认);非0:释放
* @return 0如果属性成功设置;1,否则
 *
 * @see element_meter()
